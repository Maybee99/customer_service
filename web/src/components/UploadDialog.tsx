          ...
          <div className="p-6 space-y-5">
            ...
            >
              ...
              {file ? (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-sm font-bold text-[#1E293B] truncate max-w-full">
                    {file.name}
                  </p>
                  <p className="text-xs text-slate-400">{formatSize(file.size)}</p>
                  <button
                    onClick={(e) => { e.stopPropagation(); setFile(null); }}
                    className="text-xs text-[#3B82F6] hover:underline font-semibold mt-1"
                  >
                    重新选择
                  </button>
                </div>
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-500">
                      拖拽文件到此处，或<span className="text-[#3B82F6]">点击选择</span>
                    </p>
                    <p className="text-xs text-slate-400 mt-1">
                      支持 PDF / Word (.docx)，最大 50 MB
                    </p>
                  </div>
                </div>
              )}
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-3 py-2.5 text-sm font-semibold text-[#1E293B] focus:ring-2 focus:ring-[#3B82F6]/20 focus:border-[#3B82F6] outline-none cursor-pointer appearance-none"
              >
                {CATEGORIES.map(c => (
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                onClick={handleClose}
                disabled={uploading}
                className="flex-1 px-4 py-2.5 text-sm font-bold text-[#64748B] bg-[#F1F5F9] hover:bg-[#E2E8F0] rounded-xl transition-colors cursor-pointer disabled:opacity-30"
              >
                取消
              </button>
              <button
                ...
                className={`flex-1 px-4 py-2.5 text-sm font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 ${
                  file && !uploading
                    ? 'bg-[#3B82F6] text-white hover:bg-[#2563EB] shadow-md shadow-blue-500/10 active:scale-[0.97]'
